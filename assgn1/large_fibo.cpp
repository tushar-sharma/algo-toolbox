 ...
 */
long long long fibo(int number)
{
    if (number <= 1)
        return (long long long) number;

    vector<long long long> fibo_arr(number + 1, 0);
    ...
    int i;
    for (i = 2; i <= number; i++) {
        fibo_arr[i] = (long long long) fibo_arr[i - 1] + (long long long) fibo_arr[i - 2];
    }

    return (long long long) fibo_arr[i - 1];
}
...
void tests() {
    ...
    assert( fibo(331) == 668996615388005031531000081241745415306766517246774551964595292186469);
}
